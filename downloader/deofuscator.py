...
def process_text(text, arrays):
    ...
    replacements = [deofuscator_helpers.deobfuscate_char(match, arrays[0], arrays[1]) for match in matches]
...
def process_html(html, content_info_json):
    ...
    arrays = [deofuscator_helpers.get_array(items['stbl'], items['seed']),
              deofuscator_helpers.get_array(items['ttbl'], items['seed'])]
